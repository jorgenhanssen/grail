...
impl Engine {
    ...
    pub(super) fn search_node(
        ...
        // Piece head: when NNUE was evaluated this node (no cached TT eval),
        // ask the piece head which piece type is best to move. Gives a small
        // score bonus to all moves of that piece type in both capture and quiet phases.
        let piece_hint: Option<Piece> = if tt_info.is_none()
            && self.config.nnue.value
            && self.config.piece_head_bonus.value > 0
        {
            self.nnue.as_mut().and_then(|nnue| nnue.best_piece())
        } else {
            None
        };
        ...
        let mut movegen = MainMoveGenerator::new(
            best_move_hint,
            piece_hint,
            self.config.piece_head_bonus.value,
            prev_moves,
