...
pub fn screlu(x: &Tensor) -> Result<Tensor> {
    x.contiguous()?.apply_op1(ScRelu)
}

struct ScRelu;
impl CustomOp1 for ScRelu {
    ...
    fn cuda_fwd(&self, storage: &CudaStorage, layout: &Layout) -> Result<(CudaStorage, Shape)> {
        ...
        let func = dev.get_or_load_custom_func("screlu_fwd_f32", "screlu", cuda_kernels::SCRELU)?;
    ...
    fn bwd(&self, x: &Tensor, _y: &Tensor, grad: &Tensor) -> Result<Option<Tensor>> {
        Ok(Some(
            x.contiguous()?
                .apply_op2_no_bwd(&grad.contiguous()?, &ScReluBwd)?,
        ))
    }
}

struct ScReluBwd;
impl CustomOp2 for ScReluBwd {
    fn name(&self) -> &'static str {
        "screlu_bwd"
    }
    ...
    fn cuda_fwd(
        ...
        let func = dev.get_or_load_custom_func("screlu_bwd_f32", "screlu", cuda_kernels::SCRELU)?;
        let mut builder = func.builder();
        builder.arg(&x);
        builder.arg(&gy);
        builder.arg(&gx);
